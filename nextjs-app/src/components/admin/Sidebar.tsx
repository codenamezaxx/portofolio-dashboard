        ...
        <nav className="flex-1 overflow-y-auto px-4 py-2 space-y-1">
          {NAV_ITEMS.map((item) => (
            <Link
              ...
              href={item.href}
              onClick={() => setIsOpen(false)}
              className={`flex items-center gap-3 px-3 py-2 rounded-md transition-colors text-body-sm font-medium ${isActive(item.href)
                ? 'bg-[var(--surface-card)] text-[var(--ink)] border border-[var(--hairline)]'
                : 'text-[var(--body)] hover:bg-[var(--surface-soft)] hover:text-[var(--ink)]'
                }`}
            >
              <span className="text-base w-5 flex justify-center grayscale">{item.icon}</span>
