...
const nextConfig: NextConfig = {
  ...
  async headers() {
    const cspHeader = `
      default-src 'self';
      script-src 'self' 'unsafe-eval' 'unsafe-inline' *.supabase.co *.vercel-scripts.com *.google-analytics.com;
      style-src 'self' 'unsafe-inline';
      img-src 'self' blob: data: *.supabase.co cdn.jsdelivr.net img.itch.zone *.google-analytics.com;
      font-src 'self';
      object-src 'none';
      base-uri 'self';
      form-action 'self';
      frame-ancestors 'none';
      connect-src 'self' *.supabase.co *.google-analytics.com wss://*.supabase.co;
